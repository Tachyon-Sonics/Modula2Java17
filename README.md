...
This is a command-line tool that compiles (converts) working Modula-2 sources (PIM4) into Java sources (.java files).

Do not expect miracles. I only did this tool to convert two existing Modula-2 games I wrote years ago. As such, the compiler will handle any Modula-2 construct that appears in the sources of these games, but any other constructs are very likely *not* to work properly. This project does in no way attempt to achieve full compatibility or to implement any extensions like GNU m2c for instance.
...
- Conversion of arbitrary types to `WORD` or `BYTE` is not supported. Minimal support for type-casting is provided using the `VAL(<type>, <value>)` function only.
...
- There is no graphical user interface. More generally, this tool is _not_ suited for the developement of new Modula-2 projects (due to all the previous points). The purpose is to convert legacy Modula-2 code to Java, and then to basically forget the original Modula-2 code and to continue working exclusively with the generated Java code.
...
- `ARRAY [xx .. yy]`, where `xx` and `yy` are members of an enumeration is supported. However `ARRAY <EnumType>` and more generally `ARRAY <RangeType>` are not.
- I think `BITSET` is broken. But `SET OF [0 .. 15]` should work.
- Support for compile-time constants (for instance in array bounds) is very limited.
...
A procedure used as an expression (when assigned to a variable of a procedure type), is converted to the Java 8 `module::procedure` syntax. Whenever a procedure is used as an expression, a `final` Java field with the `module::procedure` value is created next to it. This field is then used in the code instead of repeating the `module::procedure` expression everywhere. This is necessary to support equality/inequality comparison of variables of type procedure, because in Java every instance of the _same_ `module::procedure` expression results in a _different_ lambda (that are _not_ equal, even if they refer to the same class and method).
...
`NEW` and `DISPOSE` are supported. Only the version with one argument (tag variants are not supported).
...
Casting a pointer of a given type to an `ADDRESS`, and back to a pointer of the same type should work.
...
For instance, if the Modula-2 array has bounds (`ARRAY [<min>..<max>] OF CHAR`), these bounds are ignored, and array elements can be accessed past the limit. Yet the `HIGH` function will return the current size of the `String` rather than the upper bound of the original Modula-2 array.
